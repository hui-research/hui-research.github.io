...
var invest_game = {
  ...
  stimulus:function(){
    return `<h1>與他/她投資，您選擇投資多少錢？</h1> <img src="`+ pickImg[0] + `" alt="如果您看見這段文字就代表您的瀏覽器不支援圖片顯示或是網速過慢，此問題請盡快聯繫實驗者" width="500"> <p>請移動滑桿選擇數值並點擊以完成投資確認</p> `;
  },
  trial_duration: time_settings[1], // 20 秒後自動結束
  require_movement: true,
  labels: ['$0', '$1000'],
...
var invest_done = {
  ...
  <h3>整個過程預計需要約 5分鐘。</h3>
  <p>——————————</p>
  <h4>在進行過程中，請根據畫面上的指示進行操作。若您感到不適或有任何問題，您可以隨時選擇中止實驗。</h4>
  <h4>只需點擊出現在不同階段界面上的「中止實驗」按鈕，或直接關閉網頁（按下「x」），我們會尊重您的選擇。</h4>
  <h3>謝謝您的參與，請準備好開始！</h3>
  `
  ],
  show_clickable_nav: true,
...
var all_exp_done = {
  type: jsPsychInstructions,
  pages: [
  `<h2>感謝您完成實驗作業！接下來，請閱讀釋疑說明與回饋資訊。再次感謝您的參與！</h2>`
  ],
  show_clickable_nav: true,
  allow_backward: false,
  button_label_previous: '上一步',
...
function showPrompt() {
  ...
  prompt.innerHTML = `
    ...
    聯絡人/碩士生：廖彗君<br>
    email：<adress><a href="mailto:psych.research.cycu@gmail.com">psych.research.cycu@gmail.com</a></adress><br>
    指導教授：洪福建 博士</p>
...
function terminateResearch(force) {
  function doterminate(){
    ...
    jsPsych.endExperiment(`
      ...
      <div class="notice-box">聯絡人/碩士生：廖彗君<br>
      email：<adress><a href="mailto:psych.research.cycu@gmail.com">psych.research.cycu@gmail.com</a></adress><br>
      指導教授：洪福建 博士</div>
